...
from app.bluesky.actor_feed import FetchBlueskyResult, fetch_actor_feed
...
async def process_bluesky_job(job: Dict[str, Any]):
    ...
    if result.success:
        await save_result(
            job_id=job["id"],
            final_url=f"bluesky://{actor}",  # Virtual URL
...
async def process_youtube_job(job: Dict[str, Any]):
    url = job["request_url"]
    ...
    cached_result = await get_cached_result("youtube", url, CACHE_DURATION)
    if cached_result:
...
async def process_crawl4ai_job(job: Dict[str, Any]):
    url = job["request_url"]
    ...
    cached_result = await get_cached_result("crawl4ai", url, CACHE_DURATION)
    if cached_result:
        logger.info(f"Found cached result for {url}. Skipping crawl.")
        await save_result(
            job_id=job["id"],
